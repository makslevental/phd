...
\end{equation}
where for the $k$th sample $A_k$ is the optical flow\anote{opticalflow} operator representing motion (rigid and perspective shift), $H_k$ represents the blur operator, $D_k$ represents the down-sampling operator, and $\epsilon$ represents the composite noise (environment and sensor noise). We now consider the challenges and nuances of estimating motion and optical blur.

In general, for a static 3-D scene and an imaging system with 6 degrees of freedom, the 2-D optical flow caused by motion is dependent on the geometry of the 3-D scene and potentially nonlinear (due to occlusion and motion parallax).
%
This pertains to multiple image registration (alignment on a common pixel grid) for MISR where we seek to relate $X_{k+1}$ to $X_k$:
\begin{equation*}
...
\end{align}
Evaluating~\ref{eqn:opticalflowtaylor} at every pixel gives a set of linear equations that enable us to fit one of the models in table~\ref{table:flow}.
%
...
%
Note that for nonstatic scenes the registration problem becomes "exponentially" more difficult (many, many more parameters need to be estiamted).
%
Furthermore registration and super resolution are not independent since the data being used to estimate the registration transforms is blurry and noisy; to wit perfectly resolved images could be much more efficiently registered.
\begin{table*}
    \begin{center}
...
\end{equation}
where $u,v$ are horizontal and vertical spatial frequencies respectively (measured in cycles/mm), $H_{\text{diff}}$ is blur due to diffraction, $H_{\text{abr}}$ is blur due to lens aberations, and $H_{\text{int}}$ is blur due to imaging sensor integration\anote{sensorintegration} (obtained by taking the Fourier transform of the shape an individual sensor in the imaging array).
%
...
\begin{equation*}
    ...
    \end{cases}
\end{equation*}
where $\tau = \rho/\rho_c$, $\rho=\sqrt{u^2 +v^2}$, $\rho_c = 1/\lambda N$ is the radial cutoff frequency, $N$ is the f-number of the optics, and $\lambda$ is the wavelength of light being diffracted.
%
...
%
Wavelength independent blurring due to aberrations can be caused by various imperfections in the lenses such as spherical aberration, comatic aberration\anote{coma}, or astigmatism. Furthermore dispersion\anote{dispersion} blurs particulars wavelengths of light. A good model for all of these effects is\cite{10.1117.12.946501}:
\begin{equation*}
    ...
    \end{cases}
\end{equation*}
Figure~\ref{fig:mtf} shows an example OTF for an imaging system with a sensor spacing of 0.050 mm and therefore sampling frequency of 20 cycles/mm and $\rho_c = 83.3~\text{cyc}/\text{mm}$ ($F=3$ and $\lambda = 4\mu\text{m}$ i.e. near infrared).
%
Notice that $\rho_c$ is much greater than the Nyquist rate ($\frac{1}{2} \times 20~\text{cyc}/\text{mm} = 10~\text{cyc}/\text{mm}$) and so many frequencies that are within the cutoff will be aliased. This in particular can be mitigated by effectively increasing sampling rate using MISR. Notice also that like a typical transfer function the OTF is not flat and therefore attenuates high spatial frequencies. Applying gain to the image wouldn't solve the attentuation problems because of aliasing but likewise this can be resolved after the effective sampling rate is increased using MISR.
\begin{figure}
