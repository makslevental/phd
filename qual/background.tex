...
\end{equation}
where we use index $j$ to indicate the possibility of registering all samples relative to a fixed referenced image (e.g. $j=0$ the first sample) or successive/relative registration (i.e. with $j=k$).
%
There are broadly two perspectives on constructing the transformation $f$: the geometric perspective which aims to model motion as a map of the image as a whole and the morphological perspective which aims to model motion as a deformation of individual pixels.
%
These two perspectives roughly correspond to a globally or locally defined transformation    $f$.
