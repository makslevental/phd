...
Upper case plain latin $X, Y$ each denote channel $\times$ row $\times$ column \textit{tensors}\anote{tensor} representing LR and HR images respectively, with $(0,0)$ corresponding to the top left corner of the image.
%
...
%
Bolded uppercase latin $\bm{X}, \bm{Y}$ denotes batches.

Bolded lower case denote $\bm{x}, \bm{y}, \bm{z}, \mathellipsis$ denote conventional column or row vectors.
...
\begin{equation}
    X_k = (D_k \circ H_k \circ A_k) (Y) + \varepsilon
    \label{eqn:imagingmodel}
\end{equation}
where for the $k$th sample $A_k$ is the operator representing motion (affine and perspective shift) induced by optical flow\anote{opticalflow}, $H_k$ represents the blur operator, $D_k$ represents the down-sampling operator, and $\varepsilon$ represents the composite noise (environment and sensor noise). We now consider the challenges and nuances of estimating motion and blur.
