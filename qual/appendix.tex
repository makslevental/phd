...
\begin{figure}
    ...
    \end{tikzpicture}
    \caption{Rotated/aligned spherical coordinate system for computing the contour integral.}\label{fig:rotalign}
\end{figure}
%
That implies
%
...
% %
%
Finally, recalling eqn.~\eqref{eqn:sepvar}, we have the \newterm{mono-chromatic spherical wave} (due to the spherical symmetry, i.e., dependence on only \(r\) and a single frequency \(\omega\)) solution
%
\begin{equation}
...
Let \(P_0\) be a point source emitting spherical waves and an arbitrary wave-front \(S\) with radius \(r_0\).
%
According to the \newterm{Huygens-Fresnel principle} each point on \(S\) is a secondary emitter.
%
...
Suppose now take our Green's function from eqn.~\eqref{eqn:helmholtzsoln}
%
\begin{equation}
    U' = \frac{e^{\iu k s}}{s}
\end{equation}
...
Hence the integration on the right-hand side eqn.~\eqref{eqn:greenssimple}
%
\begin{equation}
...
\subsubsection{Fresnel-Kirchhoff ’s diffraction}
...
This leaves the field on \(\mathcal{A}\) alone.
%
