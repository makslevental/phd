...
Elements of the \newterm{tangent space} \(T_p (\mathbb{R}^n)\) anchored at a point \(p = (p^1, \dots, p^n) \in \mathbb{R}^n\) are called \newterm{tangent vectors}
%
\begin{equation}
    \bm{v} 
    = 
...
%
The equivalence class of \((f, U)\) is the \newterm{germ} of \(f\) at \(p\).
%
...
%
\begin{theorem}
    The linear map \(\phi\) is an \newterm{isomorphism} of vector spaces
\end{theorem}
%
