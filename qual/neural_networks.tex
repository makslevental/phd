...
\end{equation}
parameterized by \(\bm{\theta}\), for each pixel in the images:
\begin{equation}
...
The current state-of-the-art DNN solution for MISR is the Enhanced Deformable Convolutional Network architectures (EDCN) \cite{wang2019edvr}.
...
%
A deformable convolution is a convolution with an irregularly shaped and spaced kernel or a convolution.
%
It's impelemented by learning the combination of a kernel and an offset field the stores the offsets of each of the kernel values (see figure~\ref{fig:deformconv}).
%
...
\end{equation}
where \(f,g\) are learned feature mappings (i.e., they anble the TSA module to measure similarity in lower dimensional feature space).
%
