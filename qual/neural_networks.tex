...
%
These iterative back-projections are in effect repeated up-down-up sampling \textit{modules}, implemented using skip connections and deconvolutions, that minimize reconstruction error (see figure~\ref{subfig:updowndbpn}).
%
...
Attention mechanisms \cite{bahdanau2014neural} are neural networks modules that can conditionally suppress certain parts of data (either at the input layer or at intermediate layers in the network).
%
...
\end{figure*}
For example, \textit{Visual attention} (attention applied to images) can be used to improve image captioning (see figure~\ref{fig:attention}).
...
\end{figure}
\input{figures/neural_networks/space_transformer.tex}
The transformer operates operates on input \(U\) to produce output \(V\) in three phases, each of which is implemented by a module (see figure~\ref{fig:spacetransformer}): 
\begin{framed}
    \begin{enumerate}
        \item \textbf{Localization}: a regression network \(f_{\text{loc}}(\cdot)\) that regresses the transformation parameters \(\bm{\theta}\) that parameterize the transformation \(\mathcal{T}_{\bm{\theta}}\). For example the six parameters that define an affine transformation (see figure~\ref{fig:affinetransformationa}) in homogenous coordinates.
        \item \textbf{Grid generation}: a sampling grid network, which generates the set of points where the input should be sampled to produce the transformed output (see figure~\ref{fig:paramsampling}). In practice it takes \(\bm{\theta}\) as a fixed parameter and a coordinate system and generates a grid but in full generality this module can even learn the correct coordinate representation.
        \item \textbf{Sampling}: a sampling kernel \(K\) that controls sampling strength on the transformed \((n,m)\) grid:
        \begin{equation*}
...
%
In this case only the localization network of the spatial transformer is used to learn a pair of displacement mappings \(\Delta_{t+1} \coloneqq (\Delta_{t+1}x, \Delta_{t+1} y)\) (parameterized by \(\bm{\theta}\)) for each pixel in the images:
\begin{equation}
    \hat{\bm{\theta}} \coloneqq \underset{\bm{\theta}}{\text{argmin}}\left[\abs{I_t - I'_{t+1}}^2 + \lambda \mathcal{H}\left( \partial_{x,y} \Delta_{x+1} \right)\right]
\end{equation}
...
The current state-of-the-art DNN solution for MISR is the Enhanced Deformable Convolutional Network architectures\cite{wang2019edvr}.
\begin{figure}
    \includegraphics[width=.49\textwidth]{figures/neural_networks/edvr.png}
