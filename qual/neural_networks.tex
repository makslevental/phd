...
\begin{framed}
    ...
    \end{itemize}
\end{framed}
\subsubsection{SRCNN and VDSR}\label{subsubsec:vdsr}
The first successful foray of Deep Learning was the 2-layer SRCNN model\cite{Dong_2016}.
%
...
\begin{equation}
    L \coloneqq \abs{{t} - ({x} + {y})}^2
\end{equation}

\subsubsection{SRGAN}\label{subsubsec:srgan}
...
\begin{equation}
    \ell_{feat}^{\phi, j} \left( \hat{y}, y\right) = \frac{1}{H_j W_j} \abs{\phi_j\left({\hat{y}}\right) - \phi_j \left(y_c\right)}^2
\end{equation}
where \(\phi_j\) is the \(j\)th layer activation of the VGG network\cite{simonyan2014very}, \(\hat{y}\) is the output of SRResNet, and \(y_c\) is the target HR image (see figure~\ref{fig:perceptualloss}), and \(H_j, W_j\) are the dimensions of the \(j\)th layer activation.
%
...
\subsubsection{Laplacian Pyramid Super-Resolution Network}
\begin{figure}
    \centering
    \begin{adjustbox}{width=\linewidth}
        \centering
...
%
They also identify \(L_2\) loss as a source of perceptual fidelity flaws and substitute \textit{Charbonnier loss}\cite{charbonnier1994two} \(\rho(x)\) to train their network:
\begin{equation}
    \rho(x) \coloneqq \sqrt{\left(\frac{x}{\epsilon}\right)^2+ 1}
\end{equation}
...
\begin{equation}
    L({t}, \bm{y}) \coloneqq \sum_{i=1}^{log_2(r)} \rho({t}_i - {y}_i)
\end{equation}
where \({t}_i\) is the target HR image down-sampled and \(\bm{y} = (y_1, \dots, y_{\log_2{r}})\) are the outputs of LapSRN at each of the tiers.
%
