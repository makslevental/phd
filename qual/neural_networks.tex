...
Deep neural network architectures for single-image super-resolution abide a four tiered hierarchy (according to complexity) (see figure~\ref{fig:comparedeepsr}) which roughly parallels the chronological order of the innovations that have contributed to the current state of the art:
\begin{framed}
    \begin{itemize}
...
\begin{equation}
    L \coloneqq \abs{(\bm{x} + \bm{y}) - \bm{t}}^2
\end{equation}

\subsubsection{Single Up-sampling}\label{subsubsec:srgan}
...
\end{figure}
In addition to using the conventional GAN loss they add an aditional term they call \textit{perceptual loss}:
\begin{equation}
    \ell_{feat}^{\phi, j} \left( \hat{y}, y\right) = \frac{1}{H_j W_j} \abs{\phi_j\left({\hat{y}}\right) - \phi_j \left(y_c\right)}^2
\end{equation}
\(\phi_j\) is the \(j\)th layer activation of the VGG network\cite{simonyan2014very}, \(\hat{y}\) is the output of SRResNet, and \(y_c\) is the target HR image (see figure~\ref{fig:perceptualloss}), and \(H_j, W_j\) are the dimensions of the \(j\)th layer activation.
%
