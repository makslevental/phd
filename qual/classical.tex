...
% For example Alam \etal \cite{Alam2000} use weighted nearest neighbors: for every pixel to be interpolated the three nearest pixels are weighted inversely by their distance (according to HR grid distance) and then their weighted average is assigned to that pixel.
% %
% This non-uniform interpolation is then followed by application of a Wiener filter whose design is informed by the OTF of the particular imaging system they study.
%
...
%
These splines require knowledge of gradients and mediate the non-uniform sampling by using a weighted average (by area) of those gradients in adjacent Delaunay cells\anote{delauney}; to be precise they produce a Delaunay triangulation of all LR pixels and compute the gradients (see figure~\ref{fig:delauney}) according to
\begin{align*}
...
%
The algorithm operates on a sliding \newterm{estimation window}, whose dimensions \(D_x, D_y\) are chosen such that the effective sampling rate exceeds the Nyquist rate for a given \(\rho_c\).
\begin{figure}[!htbp]
    \centering
    ...
    \label{fig:wiener}
\end{figure}
The pixel values for the estimation window are a function of the filter weights of nearby LR pixels within an \newterm{observation window} whose dimensions \(W_x, W_y\) are an integer multiple of \(D_x, D_y\) (see figure~\ref{fig:wiener}).
%
...
Steering kernels (see figure~\ref{fig:steering}) address the intrinsic spatial anisotropy of images using using \newterm{kernel regression} to construct an estimator\anote{estiminterp}.
%
...
\end{equation}
where \(P\) indexes over all pixels in the HR grid and \(K\) is a \newterm{kernel function} whose purpose is to attentuate the contribution of \(\bxi\) according to distance from \(\bx\).
%
\begin{figure}[!htbp]
    \centering
...
\begin{figure}[!htbp]
    ...
    \label{fig:steering}
\end{figure}
\(K\) is any non-negative, symmetric, unimodal \cite{wand1994kernel} function parameterized by a parameter \(h\) parameter that controls the \newterm{bandwidth} or \newterm{footprint} of the kernel:
\begin{equation}
    K_h(\delx) \coloneqq \frac{1}{h}K\left( h^{-1}(\delx) \right)
\end{equation}
This bandwidth parameter \(h\) can be generalized to a \newterm{smoothing kernel} \(H\) in order to make \(K = K_H\) adaptive to the local structure of the pixels.
%
...
\begin{equation}
    ...
    \label{eqn:ibp}
\end{equation}
where \(\hat{Y}^i\) is the current estimate of the blurred HR image, \((D \circ A_k)(\hat{Y}^i)\) is the projection of the current estimate to low resolution, and \((D \circ A_k)^{-1}\left( (D \circ A_k)(\hat{Y}^i) - \Xk \right)\) is the \newterm{back-projection}.
...
\end{equation}
%
where \(A'_k\) is the relative motion matrix operator and \(\eta_k\) is the only source of new pixels (noise\anote{noise} distributed \((0, Q_k)\)-Normal).
%
...
\end{equation}
where \(s \leq k\), is a Gaussian process\anote{gp}, with mean \(\bar{\bm{y}}_{k|s} = E\left[\bm{y}_{k|s}\right]\)
and covariance
\begin{equation}
...
%
The Kalman filter proceeds in two steps: an \newterm{a priori} (before measurement) prediction step and an \newterm{a posteriori} (after measurement) update step.
%
...
\begin{equation}
    ...
    \label{eqn:gibbs}
\end{equation}
where \(B(Y)\) is called the \newterm{potential}, eqn.~\eqref{eqn:logmap} becomes
\begin{equation}
...
%
They argue that \(\hat{D} \hat{\bm{H}} \hat{\bm{A}}\) is ill-conditioned\anote{illcondition} and to that end, since the convergence rate of conjugate gradients is dependent on the condition number \cite{vanderSluis1986}, propose preconditioners\anote{precondition} to improve the convergence rate.
...
\end{equation}
%
Farsiu \etal \cite{farsiu} introduce the related \newterm{bilateral} TV norm
\begin{equation}
...
Though there are many priors (or regularizers) that have been studied in the literature, none really capture natural images in all of their variation.
%
For that we need to actually learn from real data.

\subsection{Example Based}\label{subsec:example-based}

Example based techniques \newterm{learn} a mapping from
...
\begin{figure}[!htbp]
    ...
    \label{fig:lle}
\end{figure}
Chang \etal remedy this problem by using ideas from a manifold learning\anote{manifoldlearning} technique called Locally Linear Embedding \cite{saul2000introduction} (LLE) that they call Nearest Neighbor embedding (NNE).
%
First they imagine both the space of LR patches and HR patches as manifolds\anote{manifold}.
%
Then, based on the intuition that a well-sampled smooth manifold is locally linear\anote{locallinear}, they characterize the local geometry by finding convex reconstruction weights \(W_{ij}\) of any LR patch \(x_i\) by minimizing reconstruction error:
\begin{gather}
...
%
Singh \etal \cite{singh2014} make this intuition rigorous by decomposing HR and SR patch pairs into oriented \newterm{sub-bands} (i.e., gradient orientation bins) and demonstrating that low-energy sub-bands (i.e., those that contribute weakly to the decomposition) are more strongly attenuated (see figure~\ref{fig:subbands}).
\begin{figure}[!htbp]
    \centering
...
\subsubsection{Sparse Coding}\label{subsubsec:sparsecoding}
...
%
To be precise, let \(D \coloneqq \left(\bm{d}_1, \dots, \bm{d}_K \right) \in \mathbb{R}^{n \times K}\) be an \newterm{overcomplete dictionary}\anote{overcompletedictionary} of \(K\) atoms \(\bm{d}_i\), and suppose a signal \(\by \in \mathbb{R}^n\) can be represented as a sparse linear combination of these atoms:
\begin{equation}
    \by = D\bm{\alpha}
\end{equation}
...
\subsubsection{Anchored Neighborhood Regression}
...
%
Timofte \etal call these \(P_k\) \newterm{anchored regressions} because they effectively weight the contributions of LR and HR dictionary atoms.
%
...
Inspired by the anchored regressions of ANR, Schulter \etal \cite{Schulter2015} propose a technique called Super-Resolution Forests.
%
...
%
They decide whether to actually split according to the \newterm{error reduction} in performing the split:
\begin{multline*}
...
Example based learning overcomes many of challenges involved in choosing a natural prior. 
%
