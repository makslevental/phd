...
\[
    P(\bm{y}) \propto e^{-\alpha B(\bm{y})}
\]
where $B(\bm{y})$ is called the \textit{potential}, eqn.~\ref{eqn:logmap} becomes
\begin{equation}
    \hat{\bm{y}} = \underset{\bm{y}}{\text{argmin}}\left[ \left\| \bm{X} - \hat{D} \hat{\bm{H}} \hat{\bm{A}} \bm{y} \right\|^2 +\lambda B(\bm{y})\right]
    \label{eqn:minmap}
\end{equation}
where $\lambda$ absorbs $\alpha$ from $P(\bm{y})$ and $r$ from $\varepsilon_k$.
...
%
Alternatively we can (and will) start with eqn.~\ref{eqn:minmap} and choose the regularizer $B(\bm{y})$.
%
Probably the simplest prior is the Gaussian Markov random field\anote{gmrf}.
%
This corresponds to the regularlizer taking the form
\begin{equation}
    B(\bm{y}) = \bm{y}^T Q \bm{y}
\end{equation}
where $Q$ is symmetric positive definite\anote{positivedef} (PD) and captures the covariance of adjacent pixels by its off-diagonal elements.
%
Since $Q$ is symmetric PD we can write the Cholesky decomposition $Q = U^T U$, where $U$ is upper triangular, and then eqn.~\ref{eqn:minmap} becomes
\begin{equation}
    \hat{\bm{y}} = \underset{\bm{y}}{\text{argmin}}\left[ \left\| \bm{X} - \hat{D} \hat{\bm{H}} \hat{\bm{A}} \bm{y} \right\|^2 +\lambda \left\| U\bm{y} \right\|^2 \right]
    \label{eqn:tikminmap}
\end{equation}
which is a Tikhonov regularized under-determined least squares problem.
%
Letting $G = \hat{D} \hat{\bm{H}} \hat{\bm{A}}$ the solution to eqn.~\ref{eqn:tikminmap} is
\begin{equation}
    \hat{\bm{y}} = Q^{-1} G^T \left( G Q^{-1} G^T + \lambda I \right)^{-1}\bm{X}
    \label{eqn:closedform}
\end{equation}
Nguyen et al.\cite{milanfar2001} use generalized cross-validation to determine the regularization parameter $\lambda$.
%
Rather than explicitly computing inverses in eqn.~\ref{eqn:closedform}, in practice $\hat{\bm{y}}$ is found by solving the the least squares problem via optimization  (due to the size of the matrices involved optimization is faster than inversion).
%
