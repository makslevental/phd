...
%
In this context we start with all \(\Xk\) registered to a common HR grid and consider pixel values \(Y(\bxi)\) at pixel coordinates \(\bxi \coloneqq (x, y)\) as the measured data pairs \((\bxi, Y(\bxi)\).
%
...
%
One can characterize local geometry by finding convex reconstruction weights \(W_{ij}\) of any \(x_i\) from its neighbors by minimizing reconstruction error, i.e.,\begin{gather}
...
%
To be precise, let \(D \in \mathbb{R}^{n \times K}\) be an \textit{overcomplete dictionary}\anote{overcompletedictionary} of \(K\) atoms \(\bm{d}_i \in \mathbb{R}^n\), and suppose a signal \(\by \in \mathbb{R}^n\) can be represented as a sparse linear combination of these atoms i.e.,\begin{equation}
    \by = D\bm{\alpha}
\end{equation}
