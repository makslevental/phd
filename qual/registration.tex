...
\begin{mdframed}
	...
	\end{enumerate}
\end{mdframed}
The Moravec detector then assigns a \textit{corner score} at pixel coordinate \((x,y)\) equal to \(\min_{u,v} E_{x,y}(u,v)\) in order to select for the third case; high corner scores indicate strong evidence of a corner detection.
%
...
\end{align}
%
The matrix in eqn.~\eqref{eqn:structurematrix}, called the \textit{structure tensor} or \textit{second-moment matrix} \(M\), is the quantity Harris investigated.
%
...
%
SIFT identifies scale invariant and noise robust features of an image, called \textit{keypoints}, by first finding candidate points with high local curvature at multiple scales and then culling according to heuristics.
%
...
%
An alternative but closely related method is \textit{phase correlation}, based on the Fourier shift theorem\anote{fouriershift}  (whose advantage is the availability of highly optimized algorithms for computing the Fourier transform).
%
...
%
This is called the \textit{aperture problem} or the Barber-pole illusion (see figure~\ref{fig:barberpoll}) because any motion perpendicular to image gradients \(\nabla X\) (i.e., edges) isn't captured by the constraint: to wit if
\[\mathbf{u}' \cdot \nabla X= 0\]
then
\[
...
%
A \textit{spline} is a piecewise defined polynomial function. For example a simple \textit{order} \(d+1\) spline \(s\) could be defined
\[
