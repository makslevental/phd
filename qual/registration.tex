...
%
Let \(X(x,y,t)\) be pixel intensity over time (i.e., over a sequence of images),
\[
	Q(t) \coloneqq \iint_R X(x,y,t) \dif R
\]
be the total pixel intensity over some closed image region \(R\) and assume \(Q\) is differentiable in \(t\) (we deal with discretization in the forthcoming).
%
...
%
Equation~\eqref{eqn:intcontinuity} expresses the fact that the change in total pixel intensity in the region \(R\) over time is equal to net intensity creation minus net flux through the boundary.
%
By the Stokes' theorem\anote{stokes} and interchanging the order of the integration and differentiation
%\begin{align}
...
%
Then if pixel intensity is a conserved quantity (i.e., no net creation) eqn.~\eqref{eqn:diffcontinuity} becomes
\begin{equation}
...
%
If we further assume that the intensity flow is incompressible (i.e., that intensity density remains constant as it is advected by the point sources) then \(\nabla \cdot \mathbf{u} = 0\) and eqn~\eqref{eqn:conservintent} becomes
\begin{equation}
	\pd{X}{t} + \mathbf{u} \cdot \nabla X = 0
	\label{eqn:intensityconstr}
\end{equation}
This is called the \textit{optical flow constraint equation}\cite{perez2013tv}.
%
Note that for every point in the scene eqn.~\eqref{eqn:intensityconstr} is linear in two variables (the components of \(\mathbf{u}\)).
%
It is therefore under-determined and hence a regularizer is necessary in order to uniquely determine \(\mathbf{u}\).
%
...
\begin{multline}
	E(\mathbf{u}) \coloneqq
	\int_R \left[ \|\rho(\mathbf{u})\|^2 + \alpha \left( \left\| \nabla u_1 \right\|^2 + \left\| \nabla u_2 \right\|^2\right) \right] \dif R
	\label{eqn:linearhorn}
\end{multline}
where \(\left\| \nabla u_1 \right\|^2\) is the \(L_2\) norm of \(\nabla u_1 = (\pd{u_1}{x}, \pd{u_1}{y})\).
%
...
\begin{equation}
	E'(\mathbf{u}) \coloneqq \int_R \left[ \left\| u_1 \right\|^2 + \left\| u_2 \right\|^2 + \lambda \left\| \rho'(\mathbf{u}) \right\|_1 \right] \dif R
...
\begin{multline}
	...
	\int_R \left[ \left\| u_1 \right\|^2 + \left\| u_2 \right\|^2 + \frac{1}{2\theta}\left\| \mathbf{u} - \mathbf{v} \right\|^2 + \lambda \left\| \rho'(\mathbf{v}) \right\|_1 \right] \dif R
	\label{eqn:relaxhorn}
\end{multline}
