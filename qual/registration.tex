...
%
We first cover global algorithms and then move on to local algorithms.

\subsubsection{Global Algorithms}

The most commonly used global model is the affine transformation (see figures~\ref{fig:affinetransformation}), which in homogeneous coordinates\anote{homogeneous} is defined:
\[
	\begin{bmatrix}
		ax + by + t_x \\
		cx + dy+ t_y  \\
...
\begin{equation}
	...
	\end{split}
\end{equation}
In eqns.~\eqref{eqn:affinetf} and~\eqref{eqn:projectivetf} those parameters that are not pixel coordinates \(x,y\) are estimated from the pixel coordinates of features matched in the previous step.
%
In general these systems of equations are overdetermined (by design) and a unique solution is determined using regularized least-squares.
