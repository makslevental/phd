...
\(\gradedA\) is said to \newterm{graded commutative} or \(anticommutative\) if for all \(a\in A_k, b\in A_\ell\)
\[
ab = (-1)^{k\ell} ba
\]
...
\begin{example}
    The \newterm{polynomial algebra}\tablefootnote{The algebra of polynomials in two variables with coefficients and scalars in \(\R\) where addition is degree-coefficient wise and ``foil'' multiplication}
    \(A=\R[x,y]\) is \newterm{graded by degree}; \(A_k\) consists of all homogeneous polynomials of total degree \(k\). It is a direct sum of \(A_{i}\) consisting of homogeneous polynomials of degree \(i\) e.g.
    \[
        \left( x \in A_1 \right)
        \times  
...
\begin{definition}
    ...
    With the wedge product as multiplication \(A_*\) becomes and anticommutative algebra called the \newterm{\textbf{exterior algebra}} or \newterm{\textbf{Grassmann algebra}} of covectors.
\end{definition}